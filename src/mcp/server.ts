...
import {
  ...
  ValidationMode,
  ValidationProfile,
} from "../services/enhanced-config-validator";
...
export class N8NDocumentationMCPServer {
  ...
  constructor(
    ...
    if (!dbPath) {
      ...
      throw new Error(
        "Database nodes.db not found. Please run npm run rebuild first.",
      );
    }
    ...
    this.initialized = this.initializeDatabase(dbPath).then(() => {
      ...
      logger.info(
        `MCP server initialized with ${totalTools} tools (n8n API: ${apiConfigured ? "configured" : "not configured"})`,
      );
    ...
    for (const statement of statements) {
      if (statement.trim()) {
        try {
          ...
          logger.error(
            `Failed to execute SQL statement: ${statement.substring(0, 100)}...`,
            error,
          );
          throw error;
        }
  ...
  private async validateDatabaseHealth(): Promise<void> {
    ...
    try {
      ...
      if (nodeCount.count === 0) {
        logger.error(
          "CRITICAL: Database is empty - no nodes found! Please run: npm run rebuild",
        );
        throw new Error(
          'Database is empty. Run "npm run rebuild" to populate node data.',
        );
      }
      ...
      try {
        ...
        if (!ftsExists) {
          logger.warn(
            "FTS5 table missing - search performance will be degraded. Please run: npm run rebuild",
          );
        } else {
          const ftsCount = this.db
            .prepare("SELECT COUNT(*) as count FROM nodes_fts")
            .get() as { count: number };
          if (ftsCount.count === 0) {
            logger.warn(
              "FTS5 index is empty - search will not work properly. Please run: npm run rebuild",
            );
          }
        }
      } catch (ftsError) {
        ...
        logger.warn(
          "FTS5 not available - using fallback search. For better performance, ensure better-sqlite3 is properly installed.",
        );
      }

      logger.info(
        `Database health check passed: ${nodeCount.count} nodes loaded`,
      );
    } catch (error) {
  ...
  private getDisabledTools(): Set<string> {
    ...
    if (disabledToolsEnv.length > 10000) {
      logger.warn(
        `DISABLED_TOOLS environment variable too long (${disabledToolsEnv.length} chars), truncating to 10000`,
      );
    ...
    if (tools.length > 200) {
      logger.warn(
        `DISABLED_TOOLS contains ${tools.length} tools, limiting to first 200`,
      );
  ...
  private setupHandlers(): void {
    ...
    this.server.setRequestHandler(InitializeRequestSchema, async (request) => {
      ...
      if (clientVersion && clientVersion !== negotiationResult.version) {
        logger.warn(
          `Protocol version negotiated: client requested ${clientVersion}, server will use ${negotiationResult.version}`,
          {
    ...
    this.server.setRequestHandler(ListToolsRequestSchema, async (request) => {
      ...
      const hasInstanceConfig = !!(
        this.instanceContext?.n8nApiUrl && this.instanceContext?.n8nApiKey
      );
      const isMultiTenantEnabled = process.env.ENABLE_MULTI_TENANT === "true";

      const shouldIncludeManagementTools =
        hasEnvConfig || hasInstanceConfig || isMultiTenantEnabled;
      ...
      if (shouldIncludeManagementTools) {
        ...
        logger.debug(
          `Tool listing: ${tools.length} tools available (${enabledDocTools.length} documentation + ${enabledMgmtTools.length} management)`,
          {
            hasEnvConfig,
            hasInstanceConfig,
        ...
        logger.debug(
          `Tool listing: ${tools.length} tools available (documentation only)`,
          {
            hasEnvConfig,
            hasInstanceConfig,
            isMultiTenantEnabled,
      ...
      if (disabledTools.size > 0) {
        ...
        logger.debug(
          `Filtered ${disabledTools.size} disabled tools, ${tools.length}/${totalAvailableTools} tools available`,
        );
      }
      ...
      if (isN8nClient) {
        logger.info(
          "Detected n8n client, using n8n-friendly tool descriptions",
        );
    ...
    this.server.setRequestHandler(CallToolRequestSchema, async (request) => {
      ...
      if (args && typeof args === "object" && "output" in args) {
        try {
          ...
          if (
            typeof possibleNestedData === "string" &&
            possibleNestedData.trim().startsWith("{")
          ) {
            const parsed = JSON.parse(possibleNestedData);
            if (parsed && typeof parsed === "object") {
              logger.warn(
                "Detected n8n nested output bug, attempting to extract actual arguments",
                {
                  originalArgs: args,
              ...
              if (this.validateExtractedArgs(name, parsed)) {
                ...
                logger.warn(
                  "Extracted arguments failed validation, using original args",
                  {
                    toolName: name,
                    extractedArgs: parsed,
                  },
          ...
          logger.debug(
            "Failed to parse nested output, continuing with original args",
            {
              error:
      ...
      try {
        ...
        try {
          ...
          logger.warn(
            `Failed to stringify tool result for ${name}:`,
            jsonError,
          );
        ...
        if (responseText.length > 1000000) {
          // 1MB limit
          logger.warn(
            `Tool ${name} response is very large (${responseText.length} chars), truncating`,
          );
          responseText =
            responseText.substring(0, 999000) +
        ...
        if (
          ...
          errorMessage.includes("Unknown category") ||
        ...
        if (
          name.startsWith("validate_") &&
          (errorMessage.includes("config") || errorMessage.includes("nodeType"))
        ) {
          helpfulMessage +=
  ...
  private sanitizeValidationResult(result: any, toolName: string): any {
    ...
    if (toolName === "validate_node_minimal") {
      ...
      if (!summary || typeof summary !== "object") {
        summary = {
          hasErrors: Array.isArray(sanitized.errors)
            ? sanitized.errors.length > 0
            : false,
          errorCount: Array.isArray(sanitized.errors)
            ? sanitized.errors.length
            : 0,
          warningCount: Array.isArray(sanitized.warnings)
            ? sanitized.warnings.length
            : 0,
          suggestionCount: Array.isArray(sanitized.suggestions)
            ? sanitized.suggestions.length
            : 0,
        };
      }
      ...
      const filtered = {
        nodeType: String(sanitized.nodeType || ""),
        workflowNodeType: String(
          sanitized.workflowNodeType || sanitized.nodeType || "",
        ),
        displayName: String(sanitized.displayName || ""),
        valid: Boolean(sanitized.valid),
    ...
    try {
      ...
      switch (toolName) {
        ...
        default:
          ...
          return this.validateToolParamsBasic(
            toolName,
            args,
            legacyRequiredParams || [],
          );
      }
  ...
  private validateToolParamsBasic(
    ...
    if (missing.length > 0) {
      throw new Error(
        `Missing required parameters for ${toolName}: ${missing.join(", ")}. Please provide the required parameters to use this tool.`,
      );
    }

    if (invalid.length > 0) {
      throw new Error(
        `Invalid parameters for ${toolName}: ${invalid.join(", ")}. String parameters cannot be empty.`,
      );
    }
  }
  ...
  private validateExtractedArgs(toolName: string, args: any): boolean {
    ...
    for (const requiredField of required) {
      if (!(requiredField in args)) {
        logger.debug(
          `Extracted args missing required field: ${requiredField}`,
          {
            toolName,
            extractedArgs: args,
            required,
  ...
  async executeTool(name: string, args: any): Promise<any> {
    ...
    if (disabledTools.has(name)) {
      throw new Error(
        `Tool '${name}' is disabled via DISABLED_TOOLS environment variable`,
      );
    }
    ...
    switch (name) {
      ...
      case "get_node":
        ...
        if (args.mode === "search_properties") {
          if (!args.propertyQuery) {
            throw new Error(
              "propertyQuery is required for mode=search_properties",
            );
          }
          const maxResults =
            args.maxPropertyResults !== undefined
              ? Number(args.maxPropertyResults) || 20
              : 20;
      ...
      case "validate_node":
        ...
        if (typeof args.config !== "object" || args.config === null) {
          logger.warn(
            `validate_node called with invalid config type: ${typeof args.config}`,
          );
        ...
        switch (searchMode) {
          case "by_nodes":
            if (
              ...
              throw new Error(
                "nodeTypes array is required for searchMode=by_nodes",
              );
            }
            return this.listNodeTemplates(
              args.nodeTypes,
              searchLimit,
              searchOffset,
            );
          case "by_task":
            ...
            return this.getTemplatesForTask(
              args.task,
              searchLimit,
              searchOffset,
            );
            ...
            return this.searchTemplatesByMetadata(
              {
                ...
                maxSetupMinutes: args.maxSetupMinutes
                  ? Number(args.maxSetupMinutes)
                  : undefined,
                minSetupMinutes: args.minSetupMinutes
      ...
      case "n8n_get_workflow": {
        ...
        switch (workflowMode) {
          case "details":
            return n8nHandlers.handleGetWorkflowDetails(
              args,
              this.instanceContext,
            );
          case "structure":
            return n8nHandlers.handleGetWorkflowStructure(
              args,
              this.instanceContext,
            );
          case "minimal":
            return n8nHandlers.handleGetWorkflowMinimal(
              args,
              this.instanceContext,
            );
          case "full":
      ...
      case "n8n_update_full_workflow":
        this.validateToolParams(name, args, ["id"]);
        return n8nHandlers.handleUpdateWorkflow(
          args,
          this.repository!,
          this.instanceContext,
        );
      case "n8n_update_partial_workflow":
        ...
        return handleUpdatePartialWorkflow(
          args,
          this.repository!,
          this.instanceContext,
        );
      ...
      case "n8n_autofix_workflow":
        ...
        await this.ensureInitialized();
        if (!this.repository) throw new Error("Repository not initialized");
        return n8nHandlers.handleAutofixWorkflow(
          args,
          this.repository,
          this.instanceContext,
        );
      ...
      case "n8n_executions": {
        ...
        switch (execAction) {
          ...
          case "delete":
            ...
            return n8nHandlers.handleDeleteExecution(
              args,
              this.instanceContext,
            );
          default:
            throw new Error(
              `Unknown action: ${execAction}. Valid actions: get, list, delete`,
            );
        }
      }
      ...
      case "n8n_workflow_versions":
        ...
        return n8nHandlers.handleWorkflowVersions(
          args,
          this.repository!,
          this.instanceContext,
        );
      ...
      case "n8n_deploy_template":
        ...
        if (!this.repository) throw new Error("Repository not initialized");
        return n8nHandlers.handleDeployTemplate(
          args,
  ...
  private async searchNodes(
    ...
    if (
      query.includes("n8n-nodes-base.") ||
      query.includes("@n8n/n8n-nodes-langchain.")
    ) {
    ...
    if (ftsExists) {
      ...
      logger.debug(
        `Using FTS5 search with includeExamples=${options?.includeExamples}`,
      );
  ...
  private async searchNodesFTS(
    ...
    try {
      ...
      const hasHttpRequest = scoredNodes.some(
        (n) => n.node_type === "nodes-base.httpRequest",
      );
      if (cleanedQuery.toLowerCase().includes("http") && !hasHttpRequest) {
        // FTS missed HTTP Request, fall back to LIKE search
        logger.debug(
          "FTS missed HTTP Request node, augmenting with LIKE search",
        );
  ...
  private async searchNodesFuzzy(query: string, limit: number): Promise<any> {
    ...
    if (matchingNodes.length === 0) {
      const topScores = scoredNodes
        ...
        .slice(0, 5);
      logger.debug(
        `FUZZY search for "${query}" - no matches above 400. Top scores:`,
  ...
  private calculateFuzzyScore(node: NodeRow, query: string): number {
    ...
    const nodeTypeClean = nodeTypeLower
      .replace(/^nodes-base\./, "")
    ...
    if (
      ...
      nodeTypeClean.includes(queryLower)
    ) {
    ...
    if (
      displayNameLower.startsWith(queryLower) ||
      ...
      if (options?.includeExamples) {
        for (const nodeResult of result.results) {
          try {
            ...
            logger.warn(
              `Failed to fetch examples for ${nodeResult.nodeType}:`,
              error.message,
            );
          }
        }
  ...
  private calculateRelevanceScore(node: NodeRow, query: string): number {
    ...
    const type_without_prefix = type_lower
      .replace(/^nodes-base\./, "")
      .replace(/^nodes-langchain\./, "");
    ...
    // Special boost for common primary nodes
      ...
      query_lower === "webhook" &&
      node.node_type === "nodes-base.webhook"
    ) {
      score = 900;
    } else if (
      (query_lower === "http" ||
        query_lower === "http request" ||
        query_lower === "http call") &&
      node.node_type === "nodes-base.httpRequest"
    ) {
      score = 900;
    }
    // Additional boost for multi-word queries matching primary nodes
    else if (
      query_lower.includes("http") &&
      query_lower.includes("call") &&
      node.node_type === "nodes-base.httpRequest"
    ) {
      score = 890;
    } else if (
  ...
  private rankSearchResults(
    ...
    const scoredNodes = nodes.map((node) => {
      ...
      // Word boundary match in display name
        ...
        new RegExp(`\\b${query_lower}\\b`, "i").test(node.display_name)
      ) {
        score = 700;
      }
      // Contains in display name
      ...
      if (words.length > 1) {
        ...
        const allWordsInDesc = words.every((word) =>
  ...
  private async listAITools(): Promise<any> {
    ...
    return {
      ...
      usage: {
        description:
          "These nodes have the usableAsTool property set to true, making them optimized for AI agent usage.",
...
`,
    ...
    return {
      ...
      statistics: {
        ...
        documentationCoverage:
          Math.round((stats.with_docs / stats.total) * 100) + "%",
  ...
  private async getNodeEssentials(
    nodeType: string,
    includeExamples?: boolean,
  ): Promise<any> {
    await this.ensureInitialized();
  ...
  private async getNode(
    ...
    if (!validDetailLevels.includes(detail)) {
      throw new Error(
        `get_node: Invalid detail level "${detail}". Valid options: ${validDetailLevels.join(", ")}`,
      );
    }

    if (!validModes.includes(mode)) {
      throw new Error(
        `get_node: Invalid mode "${mode}". Valid options: ${validModes.join(", ")}`,
      );
    }
  ...
  private async handleInfoMode(
    ...
    switch (detail) {
      ...
      case "standard": {
        ...
        const essentials = await this.getNodeEssentials(
          nodeType,
          includeExamples,
        );
        ...
        if (includeTypeInfo) {
          essentials.requiredProperties = this.enrichPropertiesWithTypeInfo(
            essentials.requiredProperties,
          );
          essentials.commonProperties = this.enrichPropertiesWithTypeInfo(
            essentials.commonProperties,
          );
        }
      ...
      case "full": {
        ...
        if (includeTypeInfo && fullInfo.properties) {
          fullInfo.properties = this.enrichPropertiesWithTypeInfo(
            fullInfo.properties,
          );
        }
  ...
  private async handleVersionMode(
    ...
    switch (mode) {
      ...
      case "compare":
        if (!fromVersion) {
          throw new Error(
            `get_node: fromVersion is required for compare mode (nodeType: ${nodeType})`,
          );
        }
      ...
      case "breaking":
        if (!fromVersion) {
          throw new Error(
            `get_node: fromVersion is required for breaking mode (nodeType: ${nodeType})`,
          );
        }
      ...
      case "migrations":
        if (!fromVersion || !toVersion) {
          throw new Error(
            `get_node: Both fromVersion and toVersion are required for migrations mode (nodeType: ${nodeType})`,
          );
        }
      ...
      default:
        throw new Error(
          `get_node: Unknown mode: ${mode} (nodeType: ${nodeType})`,
        );
    }
  }
    ...
    if (nodeType === "nodes-base.splitInBatches") {
      if (outputName === "done" && index === 0) {
        return {
          ...
          connectionGuidance:
            "Connect to nodes that should run AFTER the loop completes",
        };
      } else if (outputName === "loop" && index === 1) {
        return {
          description: "Current batch data for this iteration",
          connectionGuidance:
            "Connect to nodes that process items INSIDE the loop (and connect their output back to this node)",
        };
      }
    }
  ...
  private async validateNodeMinimal(
    ...
    for (const prop of properties) {
      ...
      if (
        prop.displayOptions &&
        !ConfigValidator.isPropertyVisible(prop, configWithVersion)
      ) {
        continue;
      }
  ...
  private async getToolsDocumentation(
    topic?: string,
  ...
  private async listNodeTemplates(
    nodeTypes: string[],
    limit: number = 10,
    offset: number = 0,
  ): Promise<any> {
    ...
    const result = await this.templateService.listNodeTemplates(
      nodeTypes,
      limit,
      offset,
    );
  ...
  private async getTemplate(
    ...
    const usage =
      mode === "nodes_only"
        ? "Node list for quick overview"
        : mode === "structure"
          ? "Workflow structure without full details"
  ...
  private async getTemplatesForTask(
    task: string,
    limit: number = 10,
    offset: number = 0,
  ): Promise<any> {
    ...
    const result = await this.templateService.getTemplatesForTask(
      task,
      limit,
      offset,
    );
  ...
  private async searchTemplatesByMetadata(
    filters: {
      category?: string;
    ...
    if (result.items.length === 0 && offset === 0) {
      ...
      const availableCategories =
        await this.templateService.getAvailableCategories();
      const availableAudiences =
        await this.templateService.getAvailableTargetAudiences();
  ...
  private getTaskDescription(task: string): string {
    const descriptions: Record<string, string> = {
      ai_automation:
        "AI-powered workflows using OpenAI, LangChain, and other AI tools",
      data_sync: "Synchronize data between databases, spreadsheets, and APIs",
      webhook_processing:
        "Process incoming webhooks and trigger automated actions",
      email_automation: "Send, receive, and process emails automatically",
      slack_integration:
        "Integrate with Slack for notifications and bot interactions",
  ...
  private async validateWorkflow(workflow: any, options?: any): Promise<any> {
    ...
    logger.info("Workflow validation requested", {
      ...
      connectionsType: workflow?.connections
        ...
        errors: [
          {
            ...
            details:
              "Expected: workflow.nodes = [array of node objects]. " +
              getWorkflowExampleString(),
          },
        ],
        summary: { errorCount: 1 },
      };
        ...
        errors: [
          {
            ...
            details:
              "Expected: workflow.connections = {} (can be empty object). " +
              getWorkflowExampleString(),
          },
        ],
        summary: { errorCount: 1 },
